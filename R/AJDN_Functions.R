...
#' @export
#'
second_stage_refine <-function(first_stage_changepoints,x_matrix,scale_count_list,ss_refine_alpha_tilde=1.5){
...
#' @param scale_count_list list, where each entry corresponds to a dimension, and contains a vector of the scale counts (s_{r,1} x n,...,s_{r,\delta_n})
#' for that dimension
...
time_series_normalization <-function(x_matrix,t_index_list,scale_count_list,num_cores=NULL,loud=NULL){
  ...
  if(is.null(num_cores)){
    ...
    normalization_df=foreach(dimension=seq(1,p),.combine='rbind') %dopar% {
...
#' [[1]] - sparse matrix that represents
#' [[2]] - filter matrix dictionary which specifies what scale_count (i.e. \eqn{sn}) and \eqn{t_j n} each
#' row of the filter matrix corresponds to.
#' @export
#'
#' @examples
#' filter_matrix = create_filter_matrix(1000,c(50,100))
...
create_filter_matrix <- function(n,unique_scale_counts){
  ...
  for(scale_count_i in unique_scale_counts){
    ...
    single_scale_filter_matrix=sparseMatrix(i=row_indicies,j=column_indices,x=rep(filter_entries,length(t_indexes_to_test)))
...
# Example TS for testing
set.seed(1)
x_matrix = matrix(rnorm(2000),nrow=2)
x_matrix[1,500:1000] = x_matrix[1,500:1000] + 5
s = c(.05,.06,.07)
s_prime = 1/ncol(x_matrix)
alpha = 0.05
filter_matrix_list = NULL
loud=TRUE
num_cores=NULL
output_bootstraps_and_teststat=NULL
bootstrap_normal_seed=10
cp_delete_c=.01
...
ajdn_detect_jumps<-function(x_matrix,
  ...
  if(length(unique(scale_count_list))==1){
    ...
    test_statistics=foreach(i=seq(1,p),
                            .packages = c("Matrix"),
                            .combine='rbind') %dopar% {
  ...
  # If there is only one set of scales do not need to select rows from filter matrix each time you look at a different dimension.
  if(length(unique(scale_count_list))==1){
    all_same_scales=1
  ...
  if(is.null(num_cores)){
    ...
    bootstrap_evidence=foreach(i=seq(1,p),
